...
dat <- read.csv("output/Exp3_ProcessedData.csv", header = TRUE) #need long format trial by trial data
dat <- dat %>%
...
for(p in 1:nParticipants){
  ...
  for (i in 1:numstarts) {
    ...
    #method L-BFGS-B allows boxconstraints (Byrd et al., 1995) - each variable can be given a lower and/or upper bound. First value must satisfy constraints
...
#save to csv
write.csv(AllFits, file= paste(file_name_root_data,"Experiment",experiment,"FitData_single_MLE", fitTo,".csv",sep='_'))

#extracting model outputs-----
######

#check cue labels and create new dataframe with responses and model outputs
Exp3_data <- read.csv(here("original_data/Exp3_compiled_data.csv"))

Exp3_description_check <- Descriptives(Exp3_data,outcome,cueDescription)
kable(Exp3_description_check)

Exp3_description_plot <- ggplot(data = Exp3_data, mapping = aes(x=outcome))+facet_wrap(~cueDescription)+
  geom_histogram(data = Exp3_data, mapping = aes(x=outcome,after_stat(count)), binwidth = 1, fill = "pink", colour = "red")+
...
Exp3_modelOutput <- add_discrete_predictions(Exp3_data, deltaFullOutput)
names(Exp3_modelOutput)[names(Exp3_modelOutput) == 'discretePredictions'] <- 'Delta_Predictions'

Exp3_modelOutput <- add_discrete_predictions(Exp3_modelOutput, distFullOutput)
names(Exp3_modelOutput)[names(Exp3_modelOutput) == 'discretePredictions'] <- 'Distributed_Predictions'
...
# Example usage
Exp3_modelOutput <- add_response_probabilities(Exp3_modelOutput, deltaFullOutput)
colnames(Exp3_modelOutput) <- sub("^responseProb_", "delta_responseProb_", colnames(Exp3_modelOutput))

Exp3_modelOutput <- add_response_probabilities(Exp3_modelOutput, distFullOutput)
colnames(Exp3_modelOutput) <- sub("^responseProb_", "dist_responseProb_", colnames(Exp3_modelOutput))

write_csv(Exp3_modelOutput, paste0(file_name_root_data, "Exp3_SimulatedPredictions.csv"))

Exp3_description_check <- Descriptives(Exp3_modelOutput,outcome,cueDescription)
kable(Exp3_description_check)

Exp3_description_plot <- ggplot(data = Exp3_modelOutput, mapping = aes(x=outcome))+facet_wrap(~cueDescription)+
  geom_histogram(data = Exp3_modelOutput, mapping = aes(x=outcome,after_stat(count)), binwidth = 1, fill = "pink", colour = "red")+
  # geom_freqpoly(data = Exp3_data, colour = "red",size = 1, stat ="density",linetype = "dashed")+
  theme(panel.background = element_blank(), axis.line = element_line(colour = "black"))
...
Exp3_delta_terminalV <- extract_terminalV(Exp3_data, deltaFullOutput)
Exp3_delta_terminalV <- as.data.frame(Exp3_delta_terminalV)
...
Exp3_delta_terminalV <- Exp3_delta_terminalV %>%
...
#sanity checks
Exp3_terminal_check <- Descriptives(Exp3_delta_terminalV,terminalV_plusContext,cueDescription)
kable(Exp3_terminal_check)

write_csv(Exp3_delta_terminalV, paste0(file_name_root_data, "Exp3_delta_terminalV.csv"))
...
Exp3_dist_terminalV <- extract_distribution_terminalV(Exp3_data, distFullOutput)
...
# Merge cueDescription into Exp4_delta_terminalV based on cueNum
Exp3_dist_terminalV <- Exp3_dist_terminalV %>%
  left_join(test, by = "cueNum")

write_csv(Exp3_dist_terminalV, paste0(file_name_root_data, "Exp3_dist_terminalV.csv"))
