...
dat <- read.csv("output/Exp1_ProcessedData.csv", header = TRUE) #need long format trial by trial data
dat <- dat %>%
...
nTrials <- max(dat$trialNumber) #210 training, 120 test
nTrain <- 231
...
Exp1_description_check <- Descriptives(Exp1_data,outcome,cueDescription)
kable(Exp1_description_check)


Exp1_description_plot <- ggplot(data = Exp1_data, mapping = aes(x=outcome))+facet_wrap(~cueDescription)+
  geom_histogram(data = Exp1_data, mapping = aes(x=outcome,after_stat(count)), binwidth = 1, fill = "pink", colour = "red")+
  # geom_freqpoly(data = Exp1_data, colour = "red",size = 1, stat ="density",linetype = "dashed")+
...
Exp1_modelOutput <- add_discrete_predictions(Exp1_data, deltaFullOutput)
names(Exp1_modelOutput)[names(Exp1_modelOutput) == 'discretePredictions'] <- 'Delta_Predictions'

Exp1_modelOutput <- add_discrete_predictions(Exp1_modelOutput, distFullOutput)
names(Exp1_modelOutput)[names(Exp1_modelOutput) == 'discretePredictions'] <- 'Distributed_Predictions'

Exp1_description_check <- Descriptives(Exp1_modelOutput,outcome,cueDescription)
kable(Exp1_description_check)

Exp1_description_plot <- ggplot(data = Exp1_modelOutput, mapping = aes(x=outcome))+facet_wrap(~cueDescription)+
  geom_histogram(data = Exp1_modelOutput, mapping = aes(x=outcome,after_stat(count)), binwidth = 1, fill = "pink", colour = "red")+
  # geom_freqpoly(data = Exp3_data, colour = "red",size = 1, stat ="density",linetype = "dashed")+
  theme(panel.background = element_blank(), axis.line = element_line(colour = "black"))

write_csv(Exp1_modelOutput, paste0(file_name_root_data, "Exp1_SimulatedPredictions.csv"))
...
# Example usage
Exp1_modelOutput <- add_response_probabilities(Exp1_modelOutput, deltaFullOutput)
colnames(Exp1_modelOutput) <- sub("^responseProb_", "delta_responseProb_", colnames(Exp1_modelOutput))

Exp1_modelOutput <- add_response_probabilities(Exp1_modelOutput, distFullOutput)
colnames(Exp1_modelOutput) <- sub("^responseProb_", "dist_responseProb_", colnames(Exp1_modelOutput))
...
Exp1_delta_terminalV <- extract_terminalV(Exp1_data, deltaFullOutput)
Exp1_delta_terminalV <- as.data.frame(Exp1_delta_terminalV)
...
test <- test %>% add_row(cueNum = 12, cueDescription = "context")

# Merge cueDescription into Exp4_delta_terminalV based on cueNum
Exp1_delta_terminalV <- Exp1_delta_terminalV %>%
  left_join(test, by = "cueNum")

Exp1_delta_terminalV <- Exp1_delta_terminalV %>%
  mutate(terminalV_plusContext = ifelse(cueNum == 12, 
                                        terminalV, 
                                        terminalV + terminalV[cueNum == 12][1]))

Exp1_delta_terminalV <- Exp1_delta_terminalV %>%
  select(participant,cueNum,cueDescription,terminalV,terminalV_plusContext)

#sanity checks
Exp1_terminal_check <- Descriptives(Exp1_delta_terminalV,terminalV_plusContext,cueDescription)
kable(Exp1_terminal_check)

write_csv(Exp1_delta_terminalV, paste0(file_name_root_data, "Exp1_delta_terminalV.csv"))
...
Exp1_dist_terminalV <- extract_distribution_terminalV(Exp1_data, distFullOutput)
...
# Merge cueDescription into Exp4_delta_terminalV based on cueNum
Exp1_dist_terminalV <- Exp1_dist_terminalV %>%
  left_join(test, by = "cueNum")


write_csv(Exp1_dist_terminalV, paste0(file_name_root_data, "Exp1_dist_terminalV.csv"))
