...
dat <- read.csv("output/Exp2_ProcessedData.csv", header = TRUE) #need long format trial by trial data
dat <- dat %>%
...
split_data <- split(x = dat,dat$participant)  #splits data by participant to fit on participant leve
...
AllFits <- cbind(sub_array, group_array, delta_best_pars,delta_bestValue,deltaBIC, dist_best_pars,dist_bestValue,distBIC)
#AllFits <- cbind(sub_array, delta_best_pars,delta_bestValue,deltaBIC, dist_best_pars,dist_bestValue,distBIC)

#save to csv
write.csv(AllFits, file= paste("Experiment",experiment,"FitData_single_MLE", fitTo,Sys.Date(),".csv",sep='_'))

#check cue labels
Exp2_data <- read.csv(here("original_data/Exp2_compiled_data.csv"))
  
Exp2_description_check <- Descriptives(Exp2_data,outcome,cueDescription)
kable(Exp2_description_check)

#Exp2_description_check <- Exp4_data%>%
#filter(!cueDescription%in%c("fixed0","fixed100"))

Exp2_description_plot <- ggplot(data = Exp2_data, mapping = aes(x=outcome))+facet_wrap(~cueDescription)+
  geom_histogram(data = Exp2_data, mapping = aes(x=outcome,after_stat(count)), binwidth = 1, fill = "pink", colour = "red")+
  # geom_freqpoly(data = Exp2_data, colour = "red",size = 1, stat ="density",linetype = "dashed")+
...
Exp2_modelOutput <- add_discrete_predictions(Exp2_data, deltaFullOutput)
names(Exp2_modelOutput)[names(Exp2_modelOutput) == 'discretePredictions'] <- 'Delta_Predictions'

Exp2_modelOutput <- add_discrete_predictions(Exp2_modelOutput, distFullOutput)
names(Exp2_modelOutput)[names(Exp2_modelOutput) == 'discretePredictions'] <- 'Distributed_Predictions'

Exp2_description_check <- Descriptives(Exp2_modelOutput,outcome,cueDescription)
kable(Exp2_description_check)

Exp2_description_plot <- ggplot(data = Exp2_modelOutput, mapping = aes(x=outcome))+facet_wrap(~cueDescription)+
  geom_histogram(data = Exp2_modelOutput, mapping = aes(x=outcome,after_stat(count)), binwidth = 1, fill = "pink", colour = "red")+
  # geom_freqpoly(data = Exp3_data, colour = "red",size = 1, stat ="density",linetype = "dashed")+
  theme(panel.background = element_blank(), axis.line = element_line(colour = "black"))

write_csv(Exp2_modelOutput, paste0(file_name_root_data, "Exp2_SimulatedPredictions.csv"))
...
# Example usage
Exp2_modelOutput <- add_response_probabilities(Exp2_modelOutput, deltaFullOutput)
colnames(Exp2_modelOutput) <- sub("^responseProb_", "delta_responseProb_", colnames(Exp2_modelOutput))

Exp2_modelOutput <- add_response_probabilities(Exp2_modelOutput, distFullOutput)
colnames(Exp2_modelOutput) <- sub("^responseProb_", "dist_responseProb_", colnames(Exp2_modelOutput))
...
Exp2_delta_terminalV <- extract_terminalV(Exp2_data, deltaFullOutput)
Exp2_delta_terminalV <- as.data.frame(Exp2_delta_terminalV)

#you don't want the fill items
Exp2_delta_terminalV <- Exp2_delta_terminalV %>%
  filter(cueNum%in%c(1,2,3,4,5,12,13,14))
...
test <- test %>% add_row(cueNum = 14, cueDescription = "context")
...
Exp2_delta_terminalV <- Exp2_delta_terminalV %>%
  mutate(terminalV_plusContext = ifelse(cueNum == 14, 
                                        terminalV, 
                                        terminalV + terminalV[cueNum == 14][1]))

Exp2_delta_terminalV <- Exp2_delta_terminalV %>%
  select(participant,cueNum,cueDescription,terminalV,terminalV_plusContext)

#sanity checks
Exp2_terminal_check <- Descriptives(Exp2_delta_terminalV,terminalV_plusContext,cueDescription)
kable(Exp2_terminal_check)

write_csv(Exp2_delta_terminalV, paste0(file_name_root_data, "Exp2_delta_terminalV.csv"))
...
Exp2_dist_terminalV <- extract_distribution_terminalV(Exp2_data, distFullOutput)
Exp2_dist_terminalV <- Exp2_dist_terminalV %>%
  filter(cueNum%in%c(1,2,3,4,5,12,13,14))
...
write_csv(Exp2_dist_terminalV, paste0(file_name_root_data, "Exp2_dist_terminalV.csv"))
